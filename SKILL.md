...
| 2 | Capture takes (app / web / Figma proto) | companion `screen-recording` skill if installed (its `sckrecord` is the only approved desktop/web recorder — never `screencapture -v`); simulator: `xcrun simctl io recordVideo` |
...
| Low fps whenever the page animates, on every take | Two layered causes, diagnose in order: (1) `screencapture -v` itself — its writer stalls 1–3s under any load; capture with the screen-recording skill's `sckrecord` (ScreenCaptureKit) instead, never screencapture. (2) If SCK captures are still thin during motion windows, the SCREEN is painting below 60fps — check `ps -Ao pcpu,comm -r`: a pegged WindowServer (iOS Simulator running, multiple Electron apps) starves the page's own rendering; no recorder fixes that — close the heavy apps and re-roll. Distinguish via frame DENSITY inside motion windows, not overall avg (SCK is VFR: static-hold gaps are normal) |
...
| "The phone mockup looks weird / cheap / fake" | A device frame was hand-drawn (rounded rect + uniform bezel) instead of using the shipped `assets/iphone-mockup.png` or a project PNG → law 16. Hand-drawn frames lack side buttons, rail and the heavier bottom bezel, and read as placeholder art |
| Screen corners square inside a rounded frame (or vice versa) | The plate's cleared hole and composite2's `screenBox.cornerRadius` were set from two different guesses → measure the radius from the frame PNG's alpha and pass it to BOTH (law 16). With a full-canvas plate, composite2's default `150 * s` evaluates at `s=1.0` and is almost always wrong |
| Alpha bounding box of a frame PNG returns the whole image | The area outside the device silhouette is transparent too → flood-fill the transparent mask from the border to mark exterior, then bbox what's left. A centre-out scan instead stops early on an opaque Dynamic Island and reports a too-short screen |
| Composited render is entirely black at the wide keyframes | `fx`/`fy` were written as screen-rect fractions; they are fractions of the whole FRAME_PNG → `fx = plate_px_x / plate_width`. Also check `CONTENT_H`: it defaults to 1200, so a full-canvas plate silently renders at 75% on black |
| `simctl io recordVideo` leaves a 0-byte .mov + an orphan `.sb-*` temp | SIGINT went to the shell/xcrun wrapper, not the recorder → signal the real process (`pgrep -f "Resources/bin/simctl io"`), then WAIT for finalization; a second kill during finalization re-truncates it. Verify size >0 before trusting the take |
